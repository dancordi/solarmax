...
namespace SolarMaxClient
{
    public class SolarMaxClient : ISolarMaxClient
    {
        ...
        public GetEnergyReportResult GetEnergyReport()
        {
            ...
            //connect
            if (this._Transport.Connect() == CommunicationResult.OK)
            {
                ...
                var resSend = this._Transport.Send(solarMaxRequest.ToString());
                if (resSend == CommunicationResult.OK)
                {
                    //receive
                    var resRec = this._Transport.Receive(out string rec);
        ...
        private bool Test()
        {
            ...
            if (this._Transport.Connect() == CommunicationResult.OK)
            {
                var resSend = this._Transport.Send(send_bytes);
                if (resSend == CommunicationResult.OK)
                {
                    var resRec = this._Transport.Receive(out byte[] rec_bytes);
                    if (resRec == CommunicationResult.OK)
                    {
